        ...
        if results.get("success") and results.get("available_trains"):
            ...
            if evening_trains:
                ...
                for train in evening_trains:
                    print(
                        f"   🚂 {train['train_number']}: {train['departure_time']} → {train['arrival_time']} ({train['available_seats']} seats)"
                    )
...
def search_specific_dates(dates, direction):
    ...
    print(
        f"Direction: {'Woodlands CIQ → JB Sentral' if direction == Direction.SG_TO_JB else 'JB Sentral → Woodlands CIQ'}"
    )
    print("=" * 60)
...
Examples:
  ...
  python ktmb_search.py --month 11 --year 2025 --sundays-only
    ...
    date_group.add_argument("--month", "-m", type=int, help="Month to search (1-12)")
    ...
    parser.add_argument(
        "--year", "-y", type=int, help="Year to search (required when using --month)"
    )
    ...
    if args.dates:
        # Search specific dates
        direction = (
            Direction.SG_TO_JB if args.direction == "sg-to-jb" else Direction.JB_TO_SG
        )
        friday_results = []
        sunday_results = []
    ...
    if friday_results:
        for result in friday_results:
            print(
                f"   {result['date']} ({result['day']}): {result['total_evening']} trains"
            )
    else:
